function init() {
  ...
  var oneClickBuyBasket = function () {
    ...
    if (window.matchMedia('(min-width:992px)').matches) {
      ...
      if (!document.querySelector('.fast_order').classList.contains('clicked')) {
        document.querySelector('.fast_order').classList.add('clicked');
        document.querySelector('body').querySelector('.' + name + '_frame').remove();
        document.querySelector('body').querySelector('.' + name + '_trigger').remove();
        document.querySelector('body').insertAdjacentHTML("beforeend", '<div class="' + name + '_frame popup"></div>');
        document.querySelector('body').insertAdjacentHTML("beforeend", '<div class="' + name + '_trigger"></div>');

        // document.querySelector('.' + name + '_frame').jqm({
        ...
        // });
        document.querySelector('.' + name + '_trigger').click();

        Fancybox.modal.open('/ajax/one_click_buy_basket.php')
      }
    } else {
